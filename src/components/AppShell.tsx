...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  if (status === "error") {
    return (
      <main className="app-shell mx-auto min-h-[100dvh] max-w-[560px] pt-[var(--app-safe-top)]">
        <AuthFailure
          message={
            sessionError ?? "Xavfsiz Telegram sessiyasini aniqlab bo'lmadi."
          }
        />
      </main>
    );
  ...
  return (
    <main className="app-shell mx-auto min-h-[100dvh] max-w-[560px] pb-[calc(92px+var(--app-safe-bottom))] pt-[calc(12px+var(--app-safe-top))]">
      {status === "preview" && (
        <div className="mb-3 flex items-center gap-2 rounded-[7px] border border-[var(--warning-line)] bg-[var(--warning-soft)] px-3 py-2 text-[11px] font-bold text-[var(--warning)]">
      ...
      <nav
        className="bottom-nav-frame fixed z-50 rounded-[10px] border border-[var(--line)] bg-[color:var(--surface-glass)] p-1.5 backdrop-blur-xl"
          ...
          {navItems.map((item) => {
            ...
            return (
              <Link
                ...
                className={clsx(
                  "flex h-[56px] min-w-0 flex-col items-center justify-center gap-0.5 rounded-[7px] text-[10px] font-bold transition duration-150 active:scale-[0.98]",
                  active
                    ? "text-[var(--brand)]"
              ...
              >
                <span
                  className={clsx(
                    "flex h-7 min-w-10 items-center justify-center rounded-[5px] px-2.5 transition duration-150",
